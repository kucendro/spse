...
ucitel,
knihy
INNER JOIN uci ON ID_ucitele = uci.ID_ucitele
WHERE knihy.ID_studenta = NULL;
...
GROUP BY
    ucitel.ID, uci.predmet, ucitel.prijmeni, ucitel.jmeno
HAVING
    COUNT(uci.ID_studenta) > 1;
...
CREATE TABLE student2 (ID INTEGER, jmeno VARCHAR(45));

CREATE TABLE student3 (ID INTEGER, jmeno VARCHAR(45), primary key (ID));

-- Vytvořte tabulku (knihy3 = ID, ID_studenta, nazev) s primarnnim klicem a cizím klíčem, který se odkazuje na tabulku student3 a její primární klíč.

create table knihy3 (ID integer, ID_studenta INTEGER, nazev VARCHAR(100), primary key (ID), foreign key (ID_studenta) references student3(ID));

-- vytvořte tabulku (student4 (ID, jmeno)) s primarnim klicem

create table student4 (ID INTEGER, jmeno varchar(45), primary key (ID));

create table knihy4 (ID integer, ID_studenta INTEGER, nazev VARCHAR(100));

-- V tabulce knihy4 vytvořte primární klíč

alter table knihy4 add constraint ID_pk primary key (ID);

-- v tabulce knihy4 vytvořte cizí klíč, který se odkazuje na primární klíč v tablulce student4

alter table knihy4 add constraint ID_fk foreign key (ID) references student4(ID);

-- kopírování tabulek

create table knihy2 as select * from knihy;

-- nesmí být null

alter table knihy2 modify nazev varchar(100) not null;

-- id v intervalu 1000 až 10000

alter table knihy2 add check ( ID between 1000 and 10000);
...
/* 1) Vypište příjmení učitelů, kteří učí studenty, kteří nemají půjčenou žádnou knihu (pomocí spojení tabulek join, left join a where)*/

SELECT DISTINCT
    ucitel.prijmeni AS "Příjmení učitele"
FROM
    ucitel
LEFT JOIN uci ON ucitel.ID = uci.ID_ucitele
LEFT JOIN student ON uci.ID_studenta = student.ID
LEFT JOIN knihy ON student.ID = knihy.ID_studenta
WHERE
    knihy.ID_studenta IS NULL;
...
/* 2) Vypište přijmení studentů a názvy předmětů, ve kterých dostali studenti lepší známku než je průměrná známka,
která je ze známek v tabulce učí vypočtena (průměrná známka v tabulce učí).
Tyto známky budou uvedeny u každého studenta v posledním sloupečku.*/

SELECT
    student.prijmeni AS "Příjmení studenta",
    uci.predmet AS "Předmět",
    AVG(uci.znamka) AS "Průměrná známka"
FROM
    student
INNER JOIN uci ON student.ID = uci.ID_studenta
WHERE
    uci.znamka > (SELECT AVG(znamka) FROM uci)
GROUP BY
    student.prijmeni, uci.predmet
HAVING
    AVG(uci.znamka) > (SELECT AVG(znamka) FROM uci);
...
DELIMITER //
CREATE OR REPLACE FUNCTION pocet_knih (student INT)
RETURNS INT
BEGIN
    DECLARE pocet INT;
    SELECT COUNT(*) INTO pocet FROM knihy WHERE ID_studenta = student;
    RETURN pocet;
END; //
DELIMITER ;

/*Napište dotaz, který vypíše studenty, kteří mají půjčeno více jak 2 knihy s pouzitim funkce nad*/

SELECT
    student.jmeno AS "Jméno studenta",
    student.prijmeni AS "Příjmení studenta",
    pocet_knih(student.ID) AS "Počet půjčených knih"
FROM
    student
WHERE
    pocet_knih(student.ID) > 2;
